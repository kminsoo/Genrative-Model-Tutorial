...
class CycleganBestDistiller(BaseCycleganBestDistiller):
    ...
    def __init__(self, opt):
        ...
        self.lambda_SSIM = loss_weight[0] # 1e1
        self.lambda_style = loss_weight[1] #1e4
        self.lambda_feature = loss_weight[2] #1e1
        self.lambda_tv = loss_weight[3] #1e-5
    ...
    def backward_G_Student(self):
        ...
        if self.opt.lambda_vid > 0.0:
            self.loss_G_vid = self.criterionVID(self.Sfake, teacher_image)
            self.loss_G_student += self.loss_G_vid * self.opt.lambda_vid
